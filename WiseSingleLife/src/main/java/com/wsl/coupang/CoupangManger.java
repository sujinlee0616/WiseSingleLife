...
public class CoupangManger {
	...
	@Autowired
	private static CoupangDAO dao;
	
	// 공용 DB에 연결되는 DaO 
	public List<CoupangVO> sListData(){
		System.out.println("호출확인");
		List<CoupangVO> list = new ArrayList<CoupangVO>();
		List<SearchKeywordVO> slist = dao.keywordAllData();
		for(SearchKeywordVO svo : slist){
		try{
				String star ="";
				String count ="";
				String discount ="";
				...
				Document doc =Jsoup.connect("http://www.coupang.com/np/search?component=&q="+svo.getKeyword()+"&channel=user")
				.data("Accept","text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
				.data("Accept-Encoding","br, gzip, deflate")
				.data("Host","www.coupang.com")
				...
				for(int i =0; i<code.size(); i++){
					...
					try{
						...
						System.out.println("====================");
						System.out.println("검색 번호 "+svo.getCodeNo());
						System.out.println("tag(검색어) : "+svo.getKeyword());
...
//						dao.InsertCoupang(vo);
	...
	public static void main(String[] args) {
		CoupangManger cm = new CoupangManger();
		cm.sListData();
