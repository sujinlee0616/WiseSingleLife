...
public class HomePlusManager {
	...
	@Async
	public void getDataBySearch(SearchKeywordVO skvo) {
		try {
			String encoded = URLEncoder.encode(skvo.getKeyword(), "UTF-8").replaceAll("%", "%25");
			Document hpdoc = Jsoup.connect(
					"http://www.homeplus.co.kr/app.search.HeaderSearch.ghs?comm=usr.header.search.basic4&search_query="
							+ encoded)
					.get();
			...
			try {
				for (int j = 0; j < li.size(); j++) {
					...
					try {
						/*
						 * code = item.
						 * selectFirst(".exh-comtxt .zone-btn .input-number").
						 * html();
						 */
						code = item.selectFirst(".exh-comtxt").html();
						String code1 = "jsGoodDetail('";
						int target_num = code.indexOf(code1);
						String coding = "hp" + code.substring(target_num + 14,
								(code.substring(target_num).indexOf("', ''); return") + target_num));
					...
					try {
						...
						String imgcoding = "http:" + imgcode.substring(target + 17,
								(imgcode.substring(target).indexOf("blank176") + target - 9));
