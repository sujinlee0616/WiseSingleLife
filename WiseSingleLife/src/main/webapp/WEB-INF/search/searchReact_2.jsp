...
class App extends React.Component {
  constructor(props) {
    ...
    this.removeFromSearchKeywordList = this.removeFromSearchKeywordList.bind(
      this
    );
  ...
  //더보기 버튼 클릭시 키워드와, 마트이름을 가져와서 모달창에 그릴 데이터를 정한다.
  showModalBtn(e){
      var keyword = e.target.dataset.keyword;
      var martname = e.target.dataset.martname;
  }
    ...
    return (
      <section className="ftco-search bg-light">
        <div className="container">
          <div className="row">
            <SearchBar
              setKeywordList={this.state.keyword}
              optionValue={this.state.optionValue}
              save={this.state.save}
            />
            <MartTable rows={this.state.martdata} showModalBtn={this.showModalBtn} />
            <SaveItems saveItems={this.state.saveItems} />
            <RecipeRecommend
              recipeRecommendList={this.state.recipeRecommendList}
            />
...
class MartTable extends React.Component {
  constructor(props) {
    ...
    this.state = {
      martRows: this.props.rows.map((m) => <MartRow kw_data={m} showModalBtn={m.showModalBtn} />),
    };
  }
  render() {
    return (
...
class Modal extends React.Component {
  ...
  constructor(props){
    super(props)
    this.state{
      searchKeywordList: this.state.searchKeywordList.map((m)=>{
        
      })
      martdata: this.state.martdata.map((map)=>
            ...
            <div className="col-md-12 search-wrap">
              <form action="#" className="search-property">
                <div className="row">
                  ...
                  />
                  <button className="sliderBtn" onClick={this.state}>
                    검색
                  </button>
    ...
    return (
      <div className="row">
        <div className="saved">
          ...
          <div className="savedItemArea"></div>
        </div>
      </div>
    );
  }
